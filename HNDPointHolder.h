/*
* This header is generated by classdump-dyld 1.0
* on Monday, August 15, 2016 at 11:35:25 AM Eastern Daylight Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /System/Library/CoreServices/AssistiveTouch.app/assistivetouchd
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <assistivetouchd/assistivetouchd-Structs.h>
@interface HNDPointHolder : NSObject {

	double _orbValue;
	CGPoint _point;

}

@property (assign,nonatomic) double orbValue;              //@synthesize orbValue=_orbValue - In the implementation block
@property (assign,nonatomic) CGPoint point;                //@synthesize point=_point - In the implementation block
+(id)pointHolderWithPoint:(CGPoint)arg1 orbValue:(double)arg2 ;
+(id)pointHolderWithPoint:(CGPoint)arg1 ;
-(CGPoint)point;
-(void)setPoint:(CGPoint)arg1 ;
-(double)orbValue;
-(void)setOrbValue:(double)arg1 ;
@end

